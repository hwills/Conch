...
int main(int argc, const char * argv[]) {
    ...
    int my_pipe[2];
    const int MAX_PIPE_BUFFER_SIZE = 256;
    char pipe_buffer[MAX_PIPE_BUFFER_SIZE];
    ...
    // while(true) {
        ...
        int pid;
        if((pid = fork()) == 0) {
            close(1);   // TODO: Debug, we think this closes stdinput
            // dup2(0, my_pipe[1]);
            close(my_pipe[0]);

            int value = (int(69) << 8) + 69;
            std::string my_string = "foo";
            // std::cout << "foo" << std::endl;
            write(my_pipe[1], &my_string[0], 4);
            write(my_pipe[1], &value, sizeof(value));
            _exit(0);
        }
        else {
            close(my_pipe[1]);
            ...
            read(my_pipe[0], pipe_buffer, 4);
            int value;
            read(my_pipe[0], &value, sizeof(value));
